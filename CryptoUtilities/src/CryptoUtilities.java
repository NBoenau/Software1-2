 ...
 *
 * @Nick Boenau Put your name here
 *
 */
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        // get last digit
        int remainder = copyN.divideBy10();
        if (remainder % 2 == 0) {
            b = true;
        }
        return b;
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        // cant find a witness that n is composite = n is prime (probably)
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        // generate randon nums with randomNumber (10-50 of them)
        // n is prime iff none of the numfs are a witness to n being composite
        // witness requires n > 2 and 1 < w < n - 1
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 0; i < candidates; i++) {
                ...
                //ensuring we don't call composite where w<1
                NaturalNumber random = randomNumber(nMinusTwo);
                if (random.compareTo(one) == 0) {
                    random.increment();
                } else if (random.compareTo(zero) == 0) {
                    random.increment();
                    random.increment();
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        while (b) {
            ...
            // if even, add 1 so n is odd, else add two to stay odd
            if (isEven(n)) {
                n.increment();
            } else {
                n.increment();
