 ...
 * </pre>
 *
 * @author Muhammad Abedeljaber and Nick Boenau
 *
 */
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private static int mod(int a, int b) {
        ...
        /*
         * Take advantage of qualities of an overflow, once overflow value goes
         * to INTEGER.MIN_VALUE and increments whatever is left over before the
         * overflow.
         */
        if (remainder < 0) {
            remainder += b;
    ...
    public final Pair<K, V> removeAny() {
        ...
        // TODO - fill in body - not done or don't know if it's right

        //or just [0] ?
        Iterator<Pair<K, V>> it = this.hashTable[this.hashTable.length]
                .iterator();
        Pair<K, V> removeAny = it.next();
        it.remove();
        int bucket = mod(removeAny.key().hashCode(), this.hashTable.length);
