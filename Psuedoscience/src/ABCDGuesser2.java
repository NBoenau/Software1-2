...
public final class ABCDGuesser2 {
     ...
     * @param out
     */
    private static void deJager(double w, double x, double y, double z,
            double u, SimpleWriter out) {
        ...
        for (int loopWCount = 0; loopWCount < maxLoops; loopWCount++) {

            double tempW = Math.pow(w, exponents[loopWCount]);
            for (int loopXCount = 0; loopXCount < maxLoops; loopXCount++) {

                double tempX = Math.pow(w, exponents[loopXCount]);
                for (int loopYCount = 0; loopYCount < maxLoops; loopYCount++) {

                    double tempY = Math.pow(w, exponents[loopYCount]);
                    for (int loopZCount = 0; loopZCount < maxLoops; loopZCount++) {
